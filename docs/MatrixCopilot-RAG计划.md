...
| 4 | 工作区检索 API | `POST /api/kb/search` 必带 `query`；profile 可省略/`auto`（按 query 选一个）或钉死真实 id；`retrieve(method=hybrid, top_n=4, rerank=False)`；filters：chunk + active + enabled + doc_* + **选定的** profile | `method=auto`；跨 profile 融合；写稿 | A 入库钉死 B 检索为空；停用块不可见；多模型库省略 profile 能命中匹配文档 |
| 5 | 知识库工作区 UI | `matrix.html` 增加 kb 导航；顶栏当前模型用于新建/未保存预览/写稿；召回与聊天按问题自动选模型并回写顶栏；文档徽章只读；每行切片独立保存/停用/删除；预览可「入库此块」；换模型须确认重切 | 改 compose/reply 拓扑；跨 profile 融合检索；静默 PATCH 换模型 | 顶栏 B 不能写进 A 文档块请求；召回 POST 只带 query；不必整篇保存才能入库 |
